...
{
    public async Task<GeneralResponseWithPayload<string>> AddApplicationAsync(ApplicationDTO dto)
    {
        try
        {
            ...
            return new GeneralResponseWithPayload<string>(true, "Application Added", app.Applicationid.ToString());
        }
        catch (System.Exception ex)
        {
            return new GeneralResponseWithPayload<string>(false, ex.Message, "");
        }
    }

    public async Task<IEnumerable<GeneralResponseWithPayload<string>>> AddApplicationsAsync(IEnumerable<ApplicationDTO> dtos)
    {
        List<GeneralResponseWithPayload<string>> responses = [];
    ...
    public async Task<GeneralResponseWithPayload<string>> EditApplicationAsync(ApplicationDTO dto)
    {
        try
        {
            ...
            if (app == null)
            {
                return new GeneralResponseWithPayload<string>(false, "application not found", "");
            }
            app.Applicationname = dto.Applicationname;
            app.Roleid = dto.Roleid;
            app.Userid = dto.Userid;
            await ctx.SaveChangesAsync();
            return new GeneralResponseWithPayload<string>(true, "Application updated", dto.Applicationid.ToString());
        }
        catch (System.Exception ex)
        {
            return new GeneralResponseWithPayload<string>(false, ex.Message, "");
